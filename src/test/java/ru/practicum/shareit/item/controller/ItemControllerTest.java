...
class ItemControllerTest {
    ...
    void setUp() throws Exception {
        ...
        this.mockMvc.perform(post("/users").contentType("application/json").content(objectMapper.writeValueAsString(userDto))).andDo(print()).andExpect(status().isCreated());
        this.mockMvc.perform(post("/items").contentType("application/json").header("X-Sharer-User-Id", 1).content(objectMapper.writeValueAsString(itemDto))).andDo(print()).andExpect(status().isCreated());
    }
